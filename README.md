...
The Sprint and Haste button prompts stay hidden while running in human and vampire form, including after the game refreshes their text or button icon. **Hide sprint/haste prompt** is in the **Controls / Action prompts** section. Turn off **Hide sprint/haste prompt** in the settings menu, or set `hideSprintPrompt = 0` in `settings.ini`, to restore them. Other action prompts retain game behavior, and manual HUD peek keeps running prompts hidden.
