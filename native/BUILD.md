...
The call stack is capped at eight nested frames and each prompt list at eight entries. A replacement supplied by another hook is preserved. No array elements, FText wrappers or modified ability values are retained between calls; no ability search, construction hook, polling or raw game offsets are used. `_QDNSprintConfigure(enabled, logging)` controls the hook's active flag and finite initialization through the existing Lua worker. It is available on the verified Framecore DLL with either script-dispatch profile.
